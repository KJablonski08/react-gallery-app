...
class App extends React.Component {
	...
	performSearch = (query) => {
		axios
			.get(
				`https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=${process.env.REACT_APP_KEY}&tags=${query}&per_page=16&format=json&nojsoncallback=1`
			)
			.then((response) => {
				if (query === 'elephants') {
					this.setState({
						elephants: response.data.photos.photo,
						loading: false,
					});
				} else if (query === 'lions') {
					...
					this.setState({
						leopards: response.data.photos.photo,
						loading: false,
					});
				} else if (query === 'buffalo') {
					this.setState({
						buffalo: response.data.photos.photo,
						loading: false,
					});
				} else if (query === 'rhinocerous') {
					this.setState({
						rhinocerous: response.data.photos.photo,
						loading: false,
					});
				} else {
					...
					<div className='container'>
						...
						<Switch>
							<Route
								exact
								path='/'
								render={() => <Redirect to='/rhinocerous' />}
							/>
							<Route
								path='/elephant'
								render={({ match }) =>
									this.state.loading ? (
										...
										<Gallery
											data={this.state.elephants}
											query={match.path.slice(1)}
										/>
									)
								}
							/>
